...
<body>
  ...
  <div class="page">
    ...
    <div style="border: 1px solid #000; padding: 0.3cm; margin: 0.3cm 0; page-break-inside: avoid;">
      ...
      ${character.origines?.length ? `<div style="font-size: 8pt;">${language === 'en' ? 'Faction origins' : language === 'nl' ? 'Factie-oorsprongen' : 'Origines de faction'} : ${character.origines.map((o) => translateFactionText(o, language)).join(' · ')}</div>` : ''}
      ${character.marqueCollective ? `<div style="font-size: 8pt;">${language === 'en' ? 'Collective Mark' : language === 'nl' ? 'Collectief Merk' : 'Marque collective'} : ${translateFactionText(character.marqueCollective, language)}${character.marqueCollectiveDetail ? ` — <em>${character.marqueCollectiveDetail}</em>` : ''}</div>` : ''}
